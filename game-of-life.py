...
PIXEL_SIZE = 6
OUTPUT_PATH = 'videos/vid-low.mp4'
...
class Rule30AndGameOfLife:
    def __init__(self, width, height,
        ...
        self.colors = (np.array(rgb_list, np.float32) * 255).astype(np.uint8)
    ...
    def step(self):
        return self.update_rows_and_gol_state(), self.update_decay()
...
#         for frame in tqdm.tqdm(executor.map(process_step, [animation] * NUM_FRAMES), total=NUM_FRAMES):
...
#     # Use ProcessPoolExecutor for parallel processing
#     with ProcessPoolExecutor(max_workers=NUM_THREADS) as executor:
#         # Use list to store future tasks
#         tasks = [executor.submit(process_step, animation) for _ in range(NUM_FRAMES)]
#
#         # Iterate over completed tasks, avoiding memory leak
#         for task in tqdm.tqdm(as_completed(tasks), total=NUM_FRAMES):
#             frame = task.result()
#             writer.add_frame(frame)
#
#     writer.write(OUTPUT_PATH)
